...
const SCRAPER_VERSION = '2026-07-10-v20';
...
function noteRateLimitResponse(resp, url) {
  ...
  if (rl.hits >= 2) {
    ...
    console.error('AUTO-STOPPED — server blocked you. Wait 15–30 min, refresh page, then: await ayText(50, 10)');
  }
  return true;
}
...
async function ayText(questionCount = 50, totalMinutes = 10, options = {}) {
  setTextOnlyMode();
...
function buildQuestionHtmlSnapshot() {
  const test = document.querySelector('#test');
  ...
  const css = [...document.querySelectorAll('link[rel="stylesheet"]')]
    ...
    .join('\n');
  const bodyHtml = test ? test.outerHTML : document.body.innerHTML;
...
async function saveHtmlBlock(count = 50, totalMinutes = 10, options = {}) {
  ...
  for (let i = 1; i <= count; i++) {
    ...
    const snap = buildQuestionHtmlSnapshot();
    downloadHtmlNow(snap.html, snap.filename);
    manifest.push({ step: i, file: snap.filename, ...snap.meta });
    console.log(`  ✓ ${snap.filename} | ${formatElapsed(performance.now() - t0)}`);
...
async function ay(questionCount = 50, totalMinutes = 3, options = {}) {
  const budget = applyTimingBudget(computeTimingBudget(questionCount, totalMinutes));
  logTimingBudget(budget);
...
async function scrapeFromOne(questionCount = 50, burstOrMinutes = 2500, options = {}) {
  if (!options.keepMode) setJsonScrapeMode();
  ...
  for (let i = 1; i <= times; i++) {
    ...
    const reveal =
      revealSync && revealSync !== allSnapshots[0]
        ? window.__screenshotMode || window.__textOnlyMode
          ? revealSync
...
console.log('TEXT JSON: await ayText(50, 10)  → scrape-text-output.json');
console.log('SAVE HTML: await ayHtml(50, 10)   → ccsqb-q01-idXXXX.html (like Save Page As)');
console.log('SCREENSHOTS: await fastScreenshotBlock(50)');
console.log('With images: await ay(50, 5)  — downloads/embeds PNGs (heavier, may 429)');
console.log('Screenshots: await fastScreenshotBlock(50)');
